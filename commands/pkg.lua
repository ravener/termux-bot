...
local function getDownloadURL(repo, filename)
    if repo == 'tur' then return string.format('https://tur.kcubeterm.com/%s', filename) end
...
local function getURL(repo, arch)
    ...
    return string.format(
        'https://packages-cf.termux.dev/apt/termux-%s/dists/%s/%s/binary-%s/Packages',
        repo,
        dir[1],
        dir[2],
        arch
...
local function fetch(repo, arch)
    ...
    if res.code ~= 200 then error(string.format('Non-200 Status Code: %d', res.code)) end
...
local function download(repo, arch)
    ...
    if not fs.existsSync('data') then fs.mkdirSync('data') end
...
local function fetchIndex(repo, arch)
    ...
    if not stats or (os.time() - stats.mtime.sec) > 6 * 60 * 60 then return download(repo, arch) end
...
local function humanize(size)
    if size > 1024 then return tostring(round(size / 1024, 2)) .. ' MB' end

    return tostring(round(size, 2)) .. ' KB'
end

local function pkgCommand(msg, args, meta)
    if #args < 1 then return 'Usage: `!pkg <package name> [repo=main] [arch=aarch64]`' end
    ...
    if not index[pkg] then return string.format('Package not found in `%s` repository.', repo) end

    local info = index[pkg]
    local hashes = string.format(
        '**MD5:** %s\n**SHA1:** %s\n**SHA256:** %s\n**SHA512:** %s',
        info.md5sum,
        info.sha1,
        info.sha256,
