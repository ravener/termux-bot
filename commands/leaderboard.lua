...
local function leaderboardCommand(message, args, meta)
    ...
    if not rows then return 'No results found, is this a dead server?' end

    local totalPages = math.max(round(#rows.id / 10), 1)

    if page > totalPages then return string.format('There are only **%d** pages.', totalPages) end
    ...
    for i, id in ipairs(topIDs) do
        ...
        table.insert(
            ...
            string.format(
                '- %s ❯ %s\n    => %s bit%s',
                tostring(((page - 1) * 10) + i):pad(2, 'right', '0'),
                name,
                reformatInt(points),
                points > 1 and 's' or ''
            )
    ...
    table.insert(
        ...
        string.format(
            ...
            aname,
            reformatInt(points),
            (points or 0) > 1 and 's' or ''
        )
    )
    return string.format(
        'Leaderboard (Page **%d** out of **%d**)\n```\n%s\n```',
        page,
        totalPages,
        table.concat(leaderboard, '\n')
    )
end
