...
for i, v in ipairs(fs.readdirSync('../tldr/')) do
    if v:startswith('pages.') then table.insert(languages, v:sub(7)) end
end
...
local function tldrCommand(msg, args, meta)
    ...
    if not cmd then return 'Usage `!tldr <command> [language=en]`' end
    ...
    if not page then return 'Page not found.' end
